...
public class CSignUp extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if(request.getParameter("MobNo")!=null){
            ...
            u.dob=request.getParameter("type");
            u.password=request.getParameter("pwd");
            u.mno=request.getParameter("MobNo");
            u.balance=10;
            ManagementImpl m=new ManagementImpl();
            int result=m.CopenAccount(u);
            if(result==1){
            sess.setAttribute("status","Corporate Account Opened Successfully.Go to Corporate Login Page to login.");
