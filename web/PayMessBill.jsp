...
<% response.setHeader("cache-control", "no-cache,no-store,must-revalidate");
 ...
 %>
 <% 
    if(request.getParameter("reqOTP")!=null&&request.getParameter("reqOTP").equals("1")){
            //System.out.print("Hello test 1");
            String otp=mngt.SendOtp(u.mno);
            session.setAttribute("OTP",otp);
            session.setAttribute("PageRequest","1");
...
<div style="width:60.7%;height:auto;margin-left:25%;float:top;background-color:#F5F5F5;"> 
   ...
   %>
    <p style="font-size:25px;margin-left:32%;">Pay Mess Bill Online</p>
    <%if(!(session.getAttribute("OTP")!=null&&session.getAttribute("PageRequest")!=null&&session.getAttribute("PageRequest").equals("1"))){%>
    <form style="margin-left:25%; padding: 0 8px;line-height:23px; background: #F5F5F5;" action="PayMessBill.jsp" method="post" onsubmit="return validation();">
        Select Mess:
        <select name="Hostel" id="hostel" style="margin-left: 8px;padding:6px; width:auto;"> 
            <option value="none">select</option>
            <option value="kuldeep">Periyar</option> 
            <option value="cbh">Chandrabhaga</option> 
            <option value="ship">Shipra</option> 
            <option value="koy">Koyna</option> 
            <option value="sat">Satluz</option> 
            <option value="jhel">Jhelam</option> 
            <option value="loh">Lohit</option> 
        </select><br>
        <span id="shostel" class="text-danger"></span> <br>
        <input type="hidden" name="reqOTP" value="1">
        <input type="hidden" name="PageRequest" value="1">
        Enter Amount:
        <input type="text" name="Amount" style="margin-top:6px;" id="amt"><br> 
        <span id="samt" class="text-danger"></span> <br>
        <input type="Submit" style="margin-left:145px;margin-top:20px;margin-bottom: 5px;" value="Get OTP"><br>
        </form>
        <%}%>
        <%if(session.getAttribute("OTP")!=null&&session.getAttribute("PageRequest")!=null&&session.getAttribute("PageRequest").equals("1")){%>
        <form style="margin-left:25%; padding: 0 8px;line-height:23px; background: #F5F5F5;" action="PayMessBill.jsp" method="post">
        ...
        <%    if(request.getParameter("otp")!=null){
                        ...
                        RequestDispatcher rd=request.getRequestDispatcher("PayMess");
                        ...
                        if(n.length()==4){
                            ...
                            response.sendRedirect("PayMessBill.jsp");
                        }
                        }
             }
        ...
        %>
        <input type="hidden" name="Hostel" value="<%out.print(request.getParameter("Hostel"));%>">
...
function validation(){
            ...
            if(hostel==="none"){
                document.getElementById("shostel").innerHTML="* Please Select Hostel Name";
                return false;
            }
            else
            {
              document.getElementById("shostel").innerHTML="";  
            }
            if(amt===""){
